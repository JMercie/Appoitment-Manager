...
func getUserByEmail(e string) (*tables.User, error) {
	db := database.DBConn
	var user tables.User
	if err := db.Where(&tables.User{Email: e}).Find(&user).Error; err != nil {
...
func getUserByUsername(u string) (*tables.User, error) {
	db := database.DBConn
	var user tables.User
	if err := db.Where(&tables.User{Username: u}).Find(&user).Error; err != nil {
		if gorm.IsRecordNotFoundError(err) {
			return nil, nil
		}
		return nil, err
